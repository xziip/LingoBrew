...
public:
    ...
    int GetSkippedItems() const { return mSkippedItems; }  // 跳过的文件数
    int GetScannedDirs() const { return mScannedDirs; }  // 已扫描的目录数
    int GetPendingScanDirs() const { return mPendingScans.size(); }  // 待扫描的目录数
    std::string GetCurrentFile() const { return mCurrentFile; }
    long GetCurrentFileSize() const { return mCopyFileSize; }  // 当前文件总大小
    long GetCurrentFileCopied() const { return mCopyFileCopied; }  // 当前文件已复制大小
    bool IsCopyingFile() const { return mIsCopyingFile; }  // 是否正在复制文件
